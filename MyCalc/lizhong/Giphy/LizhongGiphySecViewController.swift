//
//  LizhongGiphySecViewController.swift
//  MyCalc
//
...
class giphyViewController: UIViewController {
    ...
    var conversation: [ChatMessage] = [ChatMessage(text: "Hi there!", user: .joo), ChatMessage(text: "Tap the Giphy Button", user: .joo)]
    
    let conversationResponses: [String] = ["please stop texting me", "please stop", "dude",]
    ...
    let avatar: GiphyYYAnimatedImageView = {
        let path = Bundle(for: giphyViewController.self).path(forResource: "joo", ofType: "gif")
    ...
    override func viewDidLoad() {
        super.viewDidLoad()
        Giphy.configure(apiKey: "GySNZZrCfAWOl4WLuhrPUhyCHgQLGwjz", verificationMode: false)
        addChatView()
    ...
    func addChatView() {
        ...
        textFieldContainer.layer.shadowColor = giphyViewController.shadowColor.cgColor
        textFieldContainer.layer.shadowOpacity = giphyViewController.shadowOpacity
        ...
        collectionView.topAnchor.constraint(equalTo: view.safeTopAnchor, constant: giphyViewController.headerHeight).isActive = true
        ...
        header.layer.shadowColor = giphyViewController.shadowColor.cgColor
        header.layer.shadowOpacity = giphyViewController.shadowOpacity
    ...
    func updateChatColors(_ theme: GPHThemeType) {
        ...
        header.backgroundColor = isDark ? giphyViewController.darkHeaderColor : .white
        collectionView.reloadData()
        view.backgroundColor = isDark ? .black : .white
    }
    
    func addMessageToConversation(text: String? = nil, media: GPHMedia? = nil, user: ChatUser = .joos) {
        let indexPath = IndexPath(row: conversation.count, section: 0)
        conversation.append(ChatMessage(text: text, user: user, media: media))
        UIView.animate(withDuration: 0, animations: {[weak self] in self?.collectionView.insertItems(at: [indexPath])})
    ...
    func didSelectMedia(giphyViewController: GiphyViewController, media: GPHMedia) {
        giphyViewController.dismiss(animated: true, completion: { [weak self] in
            ...
            DispatchQueue.main.asyncAfter(deadline: .now() + 2.0) { [weak self] in
                ...
                self.addMessageToConversation(text: response, user: .joo)
            }
        })
